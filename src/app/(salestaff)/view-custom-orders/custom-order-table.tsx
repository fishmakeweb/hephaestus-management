...
export default function CustomOrderTable() {
    ...
    const totalPages = Math.ceil((filterOrder.length > 0 ? filterOrder.length : orders.length) / rowsPerPage);
    ...
    const currentRows = filterOrder.length > 0 ? filterOrder.slice(startIndex, startIndex + rowsPerPage) : orders.slice(startIndex, startIndex + rowsPerPage);
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {currentRows.map((customOrder) => (
                        <tr key={customOrder.customOrderId}>
                            ...
                            <td className="px-6 py-3 whitespace-no-wrap">{customOrder.finishDate ? new Date(customOrder.finishDate).toLocaleDateString() : 'No data'}</td>
