...
export default function OrderTable() {
    ...
    const fetchOrders = async () => {
        ...
        const filteredOrders = response.filter(order => order.orderStatus.statusId !== 1);
            ...
            <table className="min-w-full divide-y divide-gray-200 border-collapse border border-black w-full">
                <thead>
                    <tr>
                        ...
                        <th className="px-6 py-4 bg-black text-left text-xs leading-4 font-medium text-white uppercase tracking-wider">Order Date</th>
                        ...
                        <th className="px-6 py-4 bg-black text-left text-xs leading-4 font-medium text-white uppercase tracking-wider">Total Price</th>
                    </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                    {currentRows.map((order) => (
                        <tr key={order.orderId}>
                            ...
                            <td className="px-6 py-3 whitespace-no-wrap">{new Date(order.orderDate).toLocaleDateString()}</td>
                            <td className="px-6 py-3 whitespace-no-wrap">{order.orderStatus.statusDescription}</td>
                            <td className="px-6 py-3 whitespace-no-wrap">${order.totalPrice.toFixed(2)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
